...
MIGRATIONS: list[tuple[int, str, list[str]]] = [
    ...
    (90, "desktop_invite_tokens", [
        # 自部署「邀请链接」:控制台(回环)铸可复用邀请 token,局域网内的人打开
        # /Login.html?invite=<token> → 轻量注册(用户名+密码,无邮箱)自己的账号加入。
        # token 可复用 + 有 TTL;撤销 = 删该账户名下所有未过期 token。仅本地/自部署模式启用。
        "create table if not exists desktop_invite_tokens ("
        "  id bigint generated by default as identity primary key,"
        "  token_hash text unique not null,"
        ...
        "  revoked_at timestamptz"
        ")",
        "create index if not exists idx_desktop_invite_active on desktop_invite_tokens(expires_at) where revoked_at is null",
    ]),
]
