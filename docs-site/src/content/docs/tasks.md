...
「后台任务」浮窗汇聚所有耗时较长的异步操作——包括剧本导入 / 知识提取重建、AI 生图等——让你不必守在某个页面等待，随时切换到其他功能，完成后自动收到通知。
...
导入类任务后端有真实分步进度，进度条会实时推进。生图类任务 API 不提供分步百分比，只能如实显示 spinner 和已用时间。
