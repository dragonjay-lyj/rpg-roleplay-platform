...
Navigation: top bar **Settings** → **Model Parameters** (route `#settings-modelparams`).
...
- **Top-k**: Limits sampling to the K highest-probability tokens. 0 disables the parameter.
...
- **Reasoning Effort**: Shown only for reasoning-capable models (e.g. o3, DeepSeek R1). Controls inference depth: low = fast and token-efficient, high = deepest reasoning.
...
After switching a preset, the sliders below update to match. Manually adjusting any slider switches the active preset to **Custom**. Parameters are saved in real time — no confirmation required.
...
When **Explicit** or **Unrestricted** is selected, the **NSFW Intensity** slider appears. The **NSFW Additional Constraints** text field accepts free-text instructions that are injected directly into the system prompt, for example: `All characters must be 18+`.
