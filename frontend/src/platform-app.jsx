...
const SITE_ANNOUNCEMENT = {
  ...
  title: '剧情体验升级',
  lines: [
    '更贴原著：GM 更准确地跟随原著的人物、设定与剧情走向，减少自由发挥跑偏。',
    '不再剧透：你还没玩到的章节里的角色和关键事件，不会被提前透露。',
    '进度更准：剧情进度跟着你实际玩到的地方走，不再莫名跳章。',
  ],
  note: '现有存档不受影响、照常继续；想体验新系统，开一局新游戏即可。',
};
        ...
        <div style={{ padding: '4px 0 16px' }}>
          ...
          <div style={{ background: 'rgba(196,155,78,0.10)', border: '1px solid rgba(196,155,78,0.35)', borderRadius: 8, padding: '10px 14px', marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent, #c49b4e)', marginBottom: 6 }}>
              更新 · {SITE_ANNOUNCEMENT.title}
            </div>
            <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, lineHeight: 1.7, color: 'var(--text-quiet)' }}>
              {SITE_ANNOUNCEMENT.lines.map((l, i) => <li key={i}>{l}</li>)}
            </ul>
            <div style={{ fontSize: 12, color: 'var(--text-quiet)', marginTop: 6 }}>{SITE_ANNOUNCEMENT.note}</div>
          </div>
          {/* 测试期免责 */}
